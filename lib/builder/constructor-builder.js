...
function buildSerialize() {
  ...
  if (this.tlSchema.params) {
    for (var i = 0; i < this.tlSchema.params.length; i++) {
      ...
      var isFlagCheck = typeName.match(/^flags\.(\d+)\?([%\.\w<>]+)$/);
      var isFlag;
      this.tlSchema.params[i].flag = {};
      this.tlSchema.params[i].flag.isFlag = isFlag = false;
      this.tlSchema.params[i].flag.flagIndex = -1;
      if (isFlagCheck && isFlagCheck[0]) {
        this.tlSchema.params[i].flag.isFlag = isFlag = true;
        this.tlSchema.params[i].flag.flagIndex = parseInt(isFlagCheck[1], 10);
        this.tlSchema.params[i].flag.typeName = typeName = isFlagCheck[2];
        console.log(this.tlSchema.params[i])
      }
  ...
  var flags = this.tlSchema.params.filter(v => v.flag.isFlag)
  flags.forEach(param => {
...
function buildWriteObjectProperty(propertyName, typeName, isBare, isFlag) {
  var body = isFlag ? '\t\tif (this.' + propertyName + ') {\n\t' : '';
...
function buildWriteProperty(propertyName, typeName, isFlag) {
  var functionName = '__write' + utility.capitalize(propertyName);
  var body =
